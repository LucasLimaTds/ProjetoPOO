...
public class RepositorioProduto
{
    ...
    public void ConsultarProduto(int id)
    {
        ...
        if (i != -1)
        {
            Console.WriteLine($"Produto ID: {TodosProdutos[i].ID} | Nome: {TodosProdutos[i].Nome} | Preço: {TodosProdutos[i].Valor} | Fornecedor: {TodosProdutos[i].FornecedorDoProduto.Nome}");
            Console.WriteLine("-------------------------------------------------------------------");
        }
    }
}
